...
class Header extends React.Component {
    ...
    render(){
        ...
        const { title = "ASAPFoods", showBack = false, plates = 0 } = this.props

        return (
            <header className="shadow" style={{heigh:100, display:"block",boxShadow:" 0 3px 5px rgba(57, 63, 72, 0.3)"}}>
                <div id="left"><img style={{zoom:"85%",marginTop:5}} className="invert" width="40" height="40" onClick={()=> (showBack) ? window.history.back() : this.openNav()} src={`${constants.site}/src/${(showBack) ? 'icons/back':'images/menu'}.png`} /></div>
                <div id="center" style={{textAlign:"center",width:"60%"}}>
                    <h4 id="header-title" style={{lineHeight:1.5,fontSize:18}}>{title}</h4>
                </div>
                {(!showBack) ? 
                <div className="pulse" id="right" data-content={`${plates.length}`}>
                    <Link to="checkout">
